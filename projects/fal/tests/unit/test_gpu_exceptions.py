...
class TestIsGpuError:
    def test_cuda_marker(self):
        ...
        assert _is_generic_gpu_error(exc) is True
    ...
    def test_non_gpu_error(self):
        ...
        assert _is_generic_gpu_error(exc) is False
...
class TestCatchGpuExceptionsContextManager:
    ...
    def test_broad_gpu_marker(self):
        with pytest.raises(GPUException) as exc_info:
            with catch_gpu_exceptions():
    ...
    def test_gpu_exception_passthrough(self):
        """GPUException should pass through without double-wrapping."""
        original = GPUException()
        with pytest.raises(GPUException) as exc_info:
            with catch_gpu_exceptions():
                raise original
