...
def test_websocket_endpoint_requires_async_and_annotation(isolate_agent_env):
    class SyncMissingAnnotationApp(App):
        @endpoint("/ws", is_websocket=True)
        def generate_ws(self, websocket) -> None:
            return None

    app = SyncMissingAnnotationApp()
    ...
    with patch("fal.api.api.fastapi_logger") as mock_logger:
        with pytest.raises(
            ...
            match=(
                r"Websocket endpoint '.*generate_ws' at '/ws' must be defined "
                r"with async def and declare a parameter annotated as "
                r"fastapi\.WebSocket\."
            ),
        ) as exc_info:
            app._build_app()

    mock_logger.error.assert_called_once_with(str(exc_info.value))
