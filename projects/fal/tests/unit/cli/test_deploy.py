...
def _render_deploy_result_output(
    ...
    *,
    ...
    from fal.api.deploy import DeploymentResult
    from fal.cli.deploy import _render_deploy_result
    ...
    _render_deploy_result(
        ...
        DeploymentResult(
            revision="rev-123",
            app_name="my-app",
            urls={"playground": {"/": "https://fal.ai/playground"}},
...
def test_render_deploy_result_detached_does_not_claim_success():
    output = _render_deploy_result_output(False)

    assert "Deployed successfully" not in output
    assert "Deployment started (detached)" in output
    assert "still in progress" in output
    assert "rolling out" in output
    assert "rev-123" in output
    assert "fal apps runners my-app" in output
    assert "fal apps set-rev my-app <revision>" in output


def test_render_deploy_result_detached_hints_include_env_and_team():
    output = _render_deploy_result_output(False, env="dev", team="my-team")

    assert "fal apps runners my-app --env dev --team my-team" in output
    assert "fal apps set-rev my-app <revision> --env dev --team my-team" in output


def test_render_deploy_result_detached_first_deploy_reports_success():
    # There is no previous revision to roll out from, so --detach is a no-op.
    output = _render_deploy_result_output(False, is_first_deploy=True)

    assert "Deployed successfully" in output
    assert "detached" not in output
    assert "rolling out" not in output


@pytest.mark.parametrize("attach_to_deployment", [None, True])
def test_render_deploy_result_attached_reports_success(attach_to_deployment):
    output = _render_deploy_result_output(attach_to_deployment)
...
@pytest.mark.parametrize(
    "attach_to_deployment, is_first_deploy, expected_status",
    [
        (False, False, "rolling_out"),
        (False, True, "deployed"),
        (True, False, "deployed"),
        (None, False, "deployed"),
    ],
)
def test_render_deploy_result_json_status(
    attach_to_deployment, is_first_deploy, expected_status
):
    import json

    output = _render_deploy_result_output(
        attach_to_deployment,
        is_first_deploy=is_first_deploy,
        output="json",
    )
...
def _capture_console():
    ...
    return Console(file=io.StringIO(), width=100, no_color=True, legacy_windows=False)


def _deployed_result_mock():
    return MagicMock(
        revision="rev-detached",
        app_name="my-app",
        auth_mode="public",
...
@patch("fal.cli.deploy.is_first_deployment", return_value=False)
def test_deploy_detached_reports_rollout_in_progress(
    mock_is_first_deployment,
    ...
    mock_execute_prepared_deployment.return_value = _deployed_result_mock()

    console = _capture_console()
    args = mock_args(app_ref=("src/my_app/inference.py", "MyApp"))
    args.console = console
    args.attach_to_deployment = False
...
def test_deploy_with_check_detached_reports_rollout_in_progress(
    mock_isatty,
    mock_get_production_alias,
    mock_prepare_deployment,
    mock_execute_prepared_deployment,
):
    mock_get_production_alias.return_value = _production_alias()
    mock_prepare_deployment.return_value = _prepared_deployment(reset_scale=False)
    mock_execute_prepared_deployment.return_value = _deployed_result_mock()
