...
def test_fetch_auth_credentials_returns_key_creds_when_available():
    with (
        patch("fal.auth.key_credentials", return_value=("kid", "ksecret")) as key_mock,
        patch("fal.auth._fetch_access_token") as token_mock,
    ):
...
def test_fetch_auth_credentials_falls_back_to_bearer_token():
    with (
        patch("fal.auth.key_credentials", return_value=None),
        patch("fal.auth._fetch_access_token", return_value="jwt-token"),
    ):
        auth = fetch_auth_credentials()
...
def test_fetch_auth_credentials_propagates_unauthenticated():
    with (
        patch("fal.auth.key_credentials", return_value=None),
        patch("fal.auth._fetch_access_token", side_effect=UnauthenticatedException()),
    ):
...
def test_login_saves_refresh_token():
    ...
    with (
        patch("fal.auth.auth0.login", return_value=token_data) as auth0_login,
        patch("fal.auth.local.lock_token") as lock_token,
        patch("fal.auth.local.save_token") as save_token,
    ):
...
def test_logout_revokes_and_deletes_refresh_token():
    ...
    with (
        ...
        patch("fal.auth.auth0.revoke") as revoke,
        patch("fal.auth.local.lock_token") as lock_token,
        patch("fal.auth.local.delete_token") as delete_token,
    ):
